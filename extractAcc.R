...
packages <- c("plyr", "Biostrings")
ipak(packages)

# program for extracting accession numbers
...
extractAcc <- function (acc, upstream, downstream) {
  if (grepl("[A-Z]{3}[0-9]{5}\\.[0-9]{1}", acc)) {
    ...
    url <-
      paste(
        "http://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=uniprotkb&id=",
        acc,
        "&format=default&style=raw&Retrieve=Retrieve",
        sep = ""
      )
  ...
  url <-
    paste(
      "http://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=ena_sequence&id=",
      acc,
      "&format=default&style=raw&Retrieve=Retrieve",
      sep = ""
    )
  ...
  if (length(id) == 0) {
    url_uniprot <-
      paste("http://www.uniprot.org/uniprot/?query=",
            acc,
            "&columns=id&format=tab",
            sep = "")
    ...
    if (is.na(id)) {
      ...
      url_uniparc <-
        paste("http://www.uniprot.org/uniparc/?query=",
              acc_uniparc,
              "&format=tab",
              sep = "")
  ...
  if (id == "obsolete") {
    ...
    if (grepl("join", CDS)) {
      ...
      if (grepl(":", CDS)) {
        ...
        from <-
          as.numeric(sub(".+\\(([0-9]*?)[[:punct:]]{2}.+", "\\1", CDS))
      }
      if (grepl(":", CDS2)) {
        from_CDS2 <-
          as.numeric(sub(".+:([0-9]*?)[[:punct:]]{2}.+", "\\1", CDS2))
      } else {
        from_CDS2 <-
          as.numeric(sub(".+\\(([0-9]*?)[[:punct:]]{2}.+", "\\1", CDS2))
      }
      ...
      if (grepl("complement", CDS)) {
        ...
        out_CDS2 <-
          paste(out_CDS2[2:length(out_CDS2)], collapse = "")
        out_CDS2 <-
          as.character(reverseComplement(DNAString(
            x = out_CDS2,
            start = 1,
        ...
        out[2] <-
          paste(paste(out_CDS2[2:length(out_CDS2)], collapse = ""),
                out[2],
                collapse = "",
                sep = "")
      }
      ...
      if (grepl("complement", CDS)) {
        out[1] <-
          sub("Location.+",
              paste("Protein", acc, "CDS (complement)"),
              out[1])
        if (upstream != 0) {
          if (nchar(out[2]) == len + upstream + downstream) {
            ...
            out[1] <-
              paste(out[1], "-", upstream - ((len + upstream + downstream) - nchar(out[2])), "nt")
          }
        }
        if (downstream != 0) {
          if (from2 > 1) {
        ...
        if (downstream != 0) {
          if (from2 > 1) {
            out[1] <-
              paste(out[1], "+", (nchar(out[2]) - len) - (upstream), "nt")
          } else {
            out[1] <-
              paste(out[1], "+", (nchar(out[2]) - len) - (from - from2), "nt")
          }
        }
      }
    } else {
      ...
      if (grepl("complement", CDS)) {
        ...
        to <-
          as.numeric(sub(".+[[:punct:]]{2}([0-9]*?)\\)", "\\1", CDS))
        from2 <- from - downstream
        to2 <- to + upstream
      } else {
    ...
    if (prot == F) {
      ...
      if (grepl("ERROR 12 No entries found.", page)) {
        ...
        if (is.na(id)) {
          ...
          uniparc <- read.table(url_uniparc, sep = "\t", header = T)
          id_split <-
            strsplit(as.character(uniparc$UniProtKB), "; ")
          #and grab the first non-obsolete record for "id"
          id <- id_split[[1]][!grepl("obsolete", id_split[[1]])][1]
          #if this doesn't return a valid accession number, mark the entry for removal as redundant
          if (is.na(id)) {
    ...
    out[1] <-
      paste(header_split[[1]][1], id, header_split[[1]][3], sep = "|")
