...
if (opt$processors > ncores) {opt$processors <- ncores}
registerDoParallel(cores = opt$processors)

setwd(opt$out)

if (opt$upstream < 0) {opt$upstream <- 0}
if (opt$downstream < 0) {opt$downstream <- 0}
...
parresult <- foreach (i=unique(todoseq),.combine=rbind) %dopar% {
  ...
  #sequences[[i]] = out[[1]]
  #list[[i]] = out[[2]] 
  #outdf1=data.frame(out[[1]][2])
  #colnames(outdf1)=out[[1]][1]
  if (out=="obsolete") {
